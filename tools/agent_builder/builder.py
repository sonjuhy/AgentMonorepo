...
사용법 (Python API):
    ...
    result = builder.build(
        ...
        permissions=ContainerPermissions.standard(),  # 선택 (기본값: standard)
    )
...
class AgentBuilder:
    ...
    def build(
        ...
        *,
        ...
        Args:
            ...
            permissions:   컨테이너 권한 설정. None이면 standard 프리셋 사용.
        ...
        permissions = permissions or ContainerPermissions.standard()
        ...
        files = self._write_files(
            agent_dir, name, language, code, packages, port, description, permissions
        )
        ...
        result = BuildResult(
            ...
            next_steps=_make_next_steps(name, port, permissions, self._root),
        )
        return result
    ...
    def _write_files(
        ...
        vars_ = dict(
            ...
            DOCKERFILE_USER_SETUP=permissions.dockerfile_user_setup(),
            COMPOSE_SECURITY=permissions.to_compose_yaml(),
        )
...
def _make_next_steps(
    ...
    # internal 네트워크 사용 시 networks 섹션 안내
    if permissions.network == "internal":
        lines += [
            "   ※ network_mode: internal 사용 시 compose 최하단에 아래 추가:",
            "     networks:",
            "       agent_net:",
            "         driver: bridge",
            "",
