...
#
# 계정 구조:
#   cassiopeia  — 오케스트라 에이전트 전용. 전체 키 접근 허용.
#   {agent}    — 각 하위 에이전트 전용 계정. 자신의 큐·헬스만 허용.
#
# 환경변수:
#   REDIS_CASSIOPEIA_PASSWORD — cassiopeia 계정 비밀번호 (필수)
#   REDIS_COMMUNITY_PASSWORD — 모든 하위 에이전트 공통 비밀번호 (템플릿용)

# 기본 계정 비활성화 (익명 접근 차단)
user default off nopass nocommands nokeys

# ── cassiopeia: 오케스트라 에이전트 전용 (전체 접근) ──────────────────────────
user cassiopeia on >CASSIOPEIA_PASSWORD_PLACEHOLDER ~* &* +@all
