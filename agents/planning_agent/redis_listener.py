"""
Planning Agent Redis 리스너
- OrchestraManager가 agent:planning:tasks 큐에 push한 DispatchMessage를 BLPOP으로 수신
- PlanningAgent.handle_dispatch()에 위임 후 orchestra /results로 결과 보고
- agent:planning:health Redis Hash를 15초 주기로 갱신 (OrchestraManager HealthMonitor 연동)
"""
...
logger = logging.getLogger("planning_agent.redis_listener")

_QUEUE_KEY = "agent:planning:tasks"
_HEALTH_KEY = "agent:planning:health"
_HEARTBEAT_INTERVAL = 15          # 초
_BLPOP_TIMEOUT = 5                # 초 (5초마다 CancelledError 체크)


class PlanningRedisListener:
    """
    OrchestraManager ↔ PlanningAgent 연결 브리지.

    - BLPOP으로 agent:planning:tasks 큐 감시
    - PlanningAgent.handle_dispatch() 위임
    - HTTP POST {orchestra_url}/results 결과 보고
    - 15초 주기 heartbeat (agent:planning:health)
    """
    ...
    async def listen_tasks(self) -> None:
        ...
        logger.info("[PlanningRedisListener] listen_tasks 시작 (queue: %s)", _QUEUE_KEY)
        ...
        try:
            ...
            logger.info("[PlanningRedisListener] listen_tasks 정상 종료")
        except Exception as exc:
            logger.error("[PlanningRedisListener] listen_tasks 오류: %s", exc)
            raise
    ...
    async def handle_task(self, raw: str) -> None:
        """
        수신한 JSON 문자열을 파싱하고 PlanningAgent에 위임한 뒤 결과를 보고합니다.
        ...
        try:
            ...
            logger.info("[PlanningRedisListener] 태스크 수신: task_id=%s", task_id)
            ...
            logger.error("[PlanningRedisListener] JSON 파싱 실패: %s", exc)
            agent_result = {
                "task_id": task_id,
                "status": "FAILED",
                "result_data": {},
            ...
            logger.error("[PlanningRedisListener] 태스크 처리 실패 task_id=%s: %s", task_id, exc)
            agent_result = {
                "task_id": task_id,
                "status": "FAILED",
                "result_data": {},
    ...
    async def _report_result(
        ...
        for attempt in range(3):
            try:
                ...
                logger.info(
                    "[PlanningRedisListener] 결과 보고 완료: task_id=%s status=%s",
                    task_id, status,
                )
                return
            except Exception as exc:
                wait = 2 ** attempt
                logger.warning(
                    "[PlanningRedisListener] 결과 보고 실패 (attempt %d/3): %s — %ds 후 재시도",
                    attempt + 1, exc, wait,
                )
                if attempt < 2:
        ...
        logger.error(
            "[PlanningRedisListener] 결과 보고 최종 실패: task_id=%s", task_id
        )
    ...
    async def _heartbeat_loop(self) -> None:
        """
        15초 주기로 agent:planning:health Redis Hash를 갱신합니다.
        OrchestraManager HealthMonitor가 이 키를 읽어 가용 여부를 판단합니다.
        CancelledError를 수신하면 정상 종료합니다.
        """
        logger.info("[PlanningRedisListener] heartbeat 시작")
        try:
            while True:
            ...
            logger.info("[PlanningRedisListener] heartbeat 정상 종료")

    async def _update_health(self, status: str) -> None:
        """agent:planning:health Hash 필드를 업데이트합니다."""
        try:
            redis = await self._ensure_redis()
            await redis.hset(
                _HEALTH_KEY,
                mapping={
                    ...
                    "version": "2.0.0",
                    "capabilities": "analyze_task,create_plan,update_task",
                    "current_tasks": str(self._current_task_count),
                    "max_concurrency": "3",
                },
            )
        except Exception as exc:
            logger.warning("[PlanningRedisListener] heartbeat 업데이트 실패: %s", exc)
