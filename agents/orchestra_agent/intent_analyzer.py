...
7. 분류하기 어려우면 planning 에이전트를 사용하세요."""
...
class ClaudeCLIIntentAnalyzer:
    ...
    async def analyze(
        ...
        full_prompt = f"{_build_system_prompt(capabilities)}\n\n사용자 요청: {user_input}"
        ...
        return _parse_agent_messages(stdout.decode().strip(), "orchestra", set(capabilities.keys()))
