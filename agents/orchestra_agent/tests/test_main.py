...
class TestSubmitTask:
    ...
    async def test_task_pushed_to_redis(self, async_client, fake_redis):
        await async_client.post("/tasks", json={"content": "테스트", "user_id": "u1", "channel_id": "c1"})
        length = await fake_redis.llen("agent:orchestra:tasks")
        assert length == 1
...
class TestDirectDispatch:
    ...
    async def test_success_when_agent_ready(self, async_client, fake_redis):
        ...
        with patch("agents.orchestra_agent.main.ctx") as mock_ctx:
            mock_ctx.health_monitor = AsyncMock()
            mock_ctx.health_monitor.is_agent_ready = AsyncMock(return_value=(True, "OK"))
            mock_ctx.redis_client = fake_redis
            mock_ctx.manager = AsyncMock()
            mock_ctx.manager.wait_for_result = AsyncMock(return_value={
