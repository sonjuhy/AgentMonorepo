...
from agents.orchestra_agent.models import (
...
class TestWaitForResult:
    ...
    async def test_timeout_returns_failed(self, manager):
        with patch("agents.orchestra_agent.manager._BLPOP_TIMEOUT", 1):
...
class TestProcessTaskDirectResponse:
    async def test_sends_answer_to_comm_agent(self, manager, nlu_engine, fake_redis):
        ...
        assert "안녕하세요!" in msg["content"]


# ── process_task: clarification ───────────────────────────────────────────────

class TestProcessTaskClarification:
    async def test_sends_question_to_comm_agent(self, manager, nlu_engine, fake_redis):
        from shared_core.llm.interfaces import LLMUsage
        nlu_engine._provider.generate_response.return_value = (
            json.dumps({
    ...
    async def test_clarification_with_options_includes_bullet_list(self, manager, nlu_engine, fake_redis):
        from shared_core.llm.interfaces import LLMUsage
        nlu_engine._provider.generate_response.return_value = (
            json.dumps({
                "type": "clarification", "intent": "unclear",
...
class TestRequestUserApproval:
    async def test_approved(self, manager, fake_redis):
        ...
        async def _respond_to_comm(action: str):
            # comm 큐에서 승인 요청 메시지를 읽어 approval_id를 확인 후 응답
            res = await fake_redis.blpop("agent:communication:tasks", timeout=3)
            if res:
                msg = json.loads(res[1])
                approval_id = msg["task_id"]
                await fake_redis.rpush(
                    f"orchestra:approval:{approval_id}",
                    json.dumps({"action": action}),
                )

        asyncio.create_task(_respond_to_comm("approve"))
    ...
    async def test_rejected(self, manager, fake_redis):
        ...
        asyncio.create_task(_respond_to_comm("reject"))
        approved = await manager.request_user_approval(result, _BASE_TASK)
        assert approved is False

    async def test_timeout_returns_false(self, manager):
        result = {"result_data": {"summary": "대기 중"}, "agent": "file_agent"}
        with patch("agents.orchestra_agent.manager._APPROVAL_TIMEOUT_SEC", 1):
            approved = await manager.request_user_approval(result, _BASE_TASK)
        assert approved is False
...
class TestRouteSingleWithApproval:
    ...
    async def test_approved_sends_result_to_comm(self, manager, health_monitor, fake_redis):
        ...
        async def _respond():
            # 승인 요청 메시지가 올 때까지 comm 큐를 읽어 응답
            res = await fake_redis.blpop("agent:communication:tasks", timeout=3)
            if res:
                msg = json.loads(res[1])
                await fake_redis.rpush(
                    f"orchestra:approval:{msg['task_id']}",
        ...
        msg_raw = await fake_redis.lpop("agent:communication:tasks")
        assert msg_raw is not None
        assert "삭제 완료" in json.loads(msg_raw)["content"]
    ...
    async def test_rejected_does_not_forward_result(self, manager, health_monitor, fake_redis):
        ...
        msg_raw = await fake_redis.lpop("agent:communication:tasks")
        assert msg_raw is not None
        assert "취소되었습니다" in json.loads(msg_raw)["content"]
...
class TestRunPlanWithApproval:
    ...
    async def test_approved_continues_to_next_step(self, manager, fake_redis):
        ...
        async def _respond():
            # progress 메시지(requires_user_approval=False)는 건너뛰고
            # 승인 요청 메시지에만 응답
            while True:
                res = await fake_redis.blpop("agent:communication:tasks", timeout=3)
                if not res:
                    break
                msg = json.loads(res[1])
                if msg.get("requires_user_approval"):
                    await fake_redis.rpush(
                        f"orchestra:approval:{msg['task_id']}",
                        json.dumps({"action": "approve"}),
                    )
