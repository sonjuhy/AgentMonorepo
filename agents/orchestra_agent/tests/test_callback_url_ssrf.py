...
class TestValidateCallbackUrl:
    ...
    def _call(self, url: str):
        from agents.orchestra_agent.main import _validate_callback_url
        return _validate_callback_url(url)

    def test_loopback_ipv4_blocked(self):
        with patch("agents.orchestra_agent.main.socket.gethostbyname", return_value="127.0.0.1"):
            with pytest.raises(ValueError, match="내부"):
                self._call("http://localhost/hook")

    def test_private_class_a_blocked(self):
        with patch("agents.orchestra_agent.main.socket.gethostbyname", return_value="10.0.0.1"):
            with pytest.raises(ValueError, match="내부"):
                self._call("http://internal.corp/hook")

    def test_private_class_b_blocked(self):
        with patch("agents.orchestra_agent.main.socket.gethostbyname", return_value="172.16.0.5"):
            with pytest.raises(ValueError, match="내부"):
                self._call("http://172.16.0.5/hook")

    def test_private_class_c_blocked(self):
        with patch("agents.orchestra_agent.main.socket.gethostbyname", return_value="192.168.1.100"):
            with pytest.raises(ValueError, match="내부"):
                self._call("http://192.168.1.100/hook")

    def test_link_local_blocked(self):
        with patch("agents.orchestra_agent.main.socket.gethostbyname", return_value="169.254.1.1"):
            with pytest.raises(ValueError, match="내부"):
    ...
    def test_valid_https_url_passes(self):
        with patch("agents.orchestra_agent.main.socket.gethostbyname", return_value="93.184.216.34"):
            # 예외 없이 통과해야 함
...
async def test_submit_task_rejects_private_callback_url(async_client):
    ...
    with patch("agents.orchestra_agent.main.socket.gethostbyname", return_value="192.168.1.1"):
