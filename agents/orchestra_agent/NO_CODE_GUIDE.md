...
### 2단계: 빌더 실행하기
터미널을 열고 프로젝트 루트(`AgentMonorepo`)에서 다음 명령어를 실행합니다.
...
## 🌐 방법 2: 마켓플레이스 API로 원격 설치 (노코드)

만약 터미널을 열기도 귀찮거나, 이미 서비스 중인 오케스트라 서버를 재시작하지 않고 실시간으로 에이전트를 추가하고 싶다면 **API 호출 한 방(Marketplace Install)**으로 끝낼 수 있습니다.
