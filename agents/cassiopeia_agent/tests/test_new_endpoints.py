...
class TestWebhookCallback:
    ...
    async def test_callback_url_stored_in_task(self, async_client, fake_redis):
        ...
        # cassiopeia로 디스패치된 payload에 callback_url이 포함되어 있어야 함
        cass = app_context.ctx.cassiopeia_client
        cass.send_message.assert_awaited()
        payload = cass.send_message.call_args.kwargs["payload"]
        assert payload.get("callback_url") == "https://example.com/cb"
