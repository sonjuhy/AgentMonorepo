...
def sandbox_tool(mock_client):
    with patch("agents.cassiopeia_agent.sandbox_tool.SandboxClient", return_value=mock_client):
        from agents.cassiopeia_agent.sandbox_tool import SandboxTool
        tool = SandboxTool()
...
class TestExecuteAgentTask:
    ...
    async def test_non_sandbox_uses_cassiopeia(self, manager_with_sandbox, mock_cassiopeia, fake_redis):
        ...
        assert result["status"] == "COMPLETED"
        # cassiopeia.send_message()로 에이전트에 전달되어야 함 (Redis List 아님)
        mock_cassiopeia.send_message.assert_awaited()
        kwargs = mock_cassiopeia.send_message.call_args.kwargs
        assert kwargs["receiver"] == "file_agent"
    ...
    async def test_sandbox_without_tool_uses_cassiopeia(self, manager_no_sandbox, mock_cassiopeia, fake_redis):
        ...
        result = await manager_no_sandbox._execute_agent_task(
            "sandbox_agent", "task-no-tool", dispatch, timeout=5
        )

        assert result["status"] == "COMPLETED"
        # cassiopeia.send_message()로 에이전트에 전달되어야 함
        mock_cassiopeia.send_message.assert_awaited()
        kwargs = mock_cassiopeia.send_message.call_args.kwargs
        assert kwargs["receiver"] == "sandbox_agent"
...
class TestRouteSingleSandboxBypass:
    async def test_sandbox_skips_health_check(self, manager_with_sandbox, sandbox_tool_mock, fake_redis):
        ...
        """
        from agents.cassiopeia_agent.models import NLUMetadata, SingleNLUResult
    ...
    async def test_normal_agent_still_checks_health(self, manager_with_sandbox, fake_redis):
        ...
        # file_agent 미등록 → cassiopeia로 comm 에이전트에 오류 메시지 전달
        await manager_with_sandbox._route_single(nlu_result, _BASE_TASK)

        manager_with_sandbox._cassiopeia.send_message.assert_awaited()
        kwargs = manager_with_sandbox._cassiopeia.send_message.call_args.kwargs
        assert kwargs["receiver"] == "communication"
        assert "file_agent" in kwargs["payload"]["content"]
